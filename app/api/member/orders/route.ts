...
type OrderRow = { id: string; order_number: string; item_summary: string; total: number; status: string; tracking_number: string | null; created_at: string; updated_at: string };
...
export async function GET() {
  ...
  const { results: orders } = await env.DB.prepare(
    "SELECT id,order_number,item_summary,total,status,tracking_number,created_at,updated_at FROM orders WHERE user_id=? ORDER BY created_at DESC",
